...
/** Milestones tab of a WorkItem's detail page — same table+dialog pattern as
 * a Project's own Milestones tab. Optional: only used when this work is
 * handed off to someone. */
@Component({
  selector: 'app-work-item-milestones-tab',
  standalone: true,
  imports: [
  ...
  template: `
    ...
    <p-table [value]="milestones()" [loading]="loading()" dataKey="id">
      ...
      <ng-template pTemplate="emptymessage">
        <tr><td [attr.colspan]="canWrite() ? 8 : 7">No milestones — add one only if this work is handed off to someone.</td></tr>
      </ng-template>
    </p-table>
...
export class WorkItemMilestonesTabComponent implements OnInit {
  readonly workItemId = input.required<string>();
  readonly count = output<number>();

  private readonly service = inject(WorkItemMilestoneService);
  ...
  readonly editing = signal<WorkItemMilestone | null>(null);
  ...
  load() {
    this.service.list({ work_item: this.workItemId(), page_size: 200, ordering: 'target_date' })
      .subscribe({
        next: (p) => {
  ...
  private ensureLookups() {
    ...
    if (!this.tasks().length) {
      this.taskService.list({ work_item: this.workItemId(), page_size: 200 })
        .subscribe((p) => this.tasks.set(p.results));
    }
  }

  open(m: WorkItemMilestone | null) {
    this.editing.set(m);
    this.ensureLookups();
    this.form = m
  ...
  save() {
    ...
    (id ? this.service.update(id, body) : this.service.create(body)).pipe(
      switchMap((saved) => {
